...
One situation in which you will specifically not want to rely on the deleter is a handler that writes to a file: because you have no control over *when* the deleter will run, you may reserve file access for longer than you intend. In these situations, you should open the file in `vector_start()` and clean it up in `vector_finally()`, which is guaranteed to run if `vector_start()` is called.
...
- You can and should throw exceptions to signify an error unless the method is marked `noexcept` (the deleter, `vector_finally()`, and `error()`).
