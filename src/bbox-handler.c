...
static inline void wk_bbox_handler_append(wk_bbox_handler_data_t* writer, double xmin,
                                          double ymin, double xmax, double ymax) {
  if (writer->feat_id >= writer->result_size) {
    SEXP new_result = PROTECT(
        wk_bbox_handler_realloc_result(writer->result, writer->result_size * 2 + 1));
...
int wk_bbox_handler_vector_start(const wk_vector_meta_t* vector_meta,
                                 void* handler_data) {
...
int wk_bbox_handler_feature_start(const wk_vector_meta_t* meta, R_xlen_t feat_id,
                                  void* handler_data) {
  wk_bbox_handler_data_t* data = (wk_bbox_handler_data_t*)handler_data;
...
int wk_bbox_handler_geometry_start(const wk_meta_t* meta, uint32_t part_id,
                                   void* handler_data) {
  wk_bbox_handler_data_t* data = (wk_bbox_handler_data_t*)handler_data;
...
int wk_bbox_handler_coord(const wk_meta_t* meta, const double* coord, uint32_t coord_id,
                          void* handler_data) {
  wk_bbox_handler_data_t* data = (wk_bbox_handler_data_t*)handler_data;
...
SEXP wk_c_bbox_handler_new(void) {
  ...
  wk_bbox_handler_data_t* data =
      (wk_bbox_handler_data_t*)malloc(sizeof(wk_bbox_handler_data_t));
  if (data == NULL) {
...
int wk_envelope_handler_feature_start(const wk_vector_meta_t* meta, R_xlen_t feat_id,
                                      void* handler_data) {
  wk_bbox_handler_data_t* data = (wk_bbox_handler_data_t*)handler_data;
  data->xmin = R_PosInf;
  data->ymin = R_PosInf;
...
int wk_envelope_handler_geometry_start(const wk_meta_t* meta, uint32_t part_id,
                                       void* handler_data) {
  wk_bbox_handler_data_t* data = (wk_bbox_handler_data_t*)handler_data;
...
int wk_envelope_handler_feature_end(const wk_vector_meta_t* meta, R_xlen_t feat_id,
                                    void* handler_data) {
  wk_bbox_handler_data_t* data = (wk_bbox_handler_data_t*)handler_data;
  wk_bbox_handler_append(data, data->xmin, data->ymin, data->xmax, data->ymax);
  return WK_CONTINUE;
