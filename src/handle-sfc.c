...
int wk_sfc_read_point(SEXP x, wk_handler_t* handler, uint32_t part_id);
int wk_sfc_read_linestring(SEXP x, wk_handler_t* handler, uint32_t part_id);
int wk_sfc_read_polygon(SEXP x, wk_handler_t* handler, uint32_t part_id);
...
int wk_sfc_read_sfg(SEXP x, wk_handler_t* handler, uint32_t part_id) {
    if (Rf_inherits(x, "POINT")) {
        return wk_sfc_read_point(x, handler, part_id);
    } else if (Rf_inherits(x, "LINESTRING")) {
        return wk_sfc_read_linestring(x, handler, part_id);
    } else if (Rf_inherits(x, "POLYGON")) {
        return wk_sfc_read_polygon(x, handler, part_id);
...
void wk_meta_from_sfg(SEXP x, wk_meta_t* meta) {
    if (Rf_inherits(x, "XY")) {
