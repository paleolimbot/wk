...
char wk_handler_void_vector_start(const WKGeometryMeta* meta, void* userData) {
  return WK_CONTINUE;
}

SEXP wk_handler_void_vector_end(const WKGeometryMeta* meta, void* userData) {
  return R_NilValue;
}

char wk_handler_void_feature(const WKGeometryMeta* meta, R_xlen_t nFeatures, R_xlen_t featureId, void* userData) {
  return WK_CONTINUE;
}

char wk_handler_void_geometry(const WKGeometryMeta* meta, uint32_t nParts, uint32_t partId, void* userData) {
  return WK_CONTINUE;
}

char wk_handler_void_ring(const WKGeometryMeta* meta, uint32_t nRings, uint32_t ringId, void* userData) {
  return WK_CONTINUE;
}

char wk_handler_void_coord(const WKGeometryMeta* meta, const WKCoord coord, uint32_t nCoords, uint32_t coordId, void* userData) {
  return WK_CONTINUE;
}
...
WKHandler* wk_handler_create() {
  WKHandler* handler = (WKHandler*) malloc(sizeof(WKHandler));
...
void wk_handler_destroy_xptr(SEXP xptr) {
  wk_handler_destroy((WKHandler*) R_ExternalPtrAddr(xptr));
}
