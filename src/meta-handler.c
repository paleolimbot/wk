...
int wk_meta_handler_vector_start(const wk_vector_meta_t* vector_meta, void* handler_data) {
    ...
    meta_handler->container = PROTECT(Rf_mkNamed(VECSXP, names));
    SEXP geometry_type = PROTECT(Rf_allocVector(INTSXP, vector_meta->size));
    SEXP has_z = PROTECT(Rf_allocVector(LGLSXP, vector_meta->size));
    SEXP has_m = PROTECT(Rf_allocVector(LGLSXP, vector_meta->size));
    SEXP srid = PROTECT(Rf_allocVector(INTSXP, vector_meta->size));
    SEXP size = PROTECT(Rf_allocVector(INTSXP, vector_meta->size));

    SET_VECTOR_ELT(meta_handler->container, 0, geometry_type);
    SET_VECTOR_ELT(meta_handler->container, 1, has_z);
    SET_VECTOR_ELT(meta_handler->container, 2, has_m);
    SET_VECTOR_ELT(meta_handler->container, 3, srid);
    SET_VECTOR_ELT(meta_handler->container, 4, size);

    meta_handler->geometry_type = INTEGER(geometry_type);
    meta_handler->has_z = LOGICAL(has_z);
    meta_handler->has_m = LOGICAL(has_m);
    meta_handler->srid = INTEGER(srid);
    meta_handler->size = INTEGER(size);
    
    UNPROTECT(5);

    // make output a data.frame
    Rf_setAttrib(meta_handler->container, R_ClassSymbol, Rf_mkString("data.frame"));

    // set row.names
    SEXP row_names = PROTECT(Rf_allocVector(INTSXP, 2));
    INTEGER(row_names)[0] = NA_INTEGER;
    INTEGER(row_names)[1] = vector_meta->size;
    Rf_setAttrib(meta_handler->container, Rf_install("row.names"), row_names);
    UNPROTECT(1);
