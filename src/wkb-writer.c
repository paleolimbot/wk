...
int wkb_writer_feature_start(const wk_meta_t* meta, R_xlen_t feat_id, void* handler_data) {
    wkb_write_buffer_t* write_buffer = (wkb_write_buffer_t*) handler_data;
    write_buffer->offset = 0;
    write_buffer->recursion_level = 0;
    return WK_CONTINUE;
...
int wkb_writer_feature_null(const wk_meta_t* meta, R_xlen_t feat_id, void* handler_data) {
    wkb_write_buffer_t* write_buffer = (wkb_write_buffer_t*) handler_data;
    SET_VECTOR_ELT(write_buffer->result, feat_id, R_NilValue);
    return WK_ABORT_FEATURE;
}

int wkb_writer_feature_end(const wk_meta_t* meta, R_xlen_t feat_id, void* handler_data) {
    wkb_write_buffer_t* write_buffer = (wkb_write_buffer_t*) handler_data;
