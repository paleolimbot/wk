...
SEXP wk_trans_filter_vector_end(const wk_vector_meta_t* meta, void* handler_data) {
  ...
  return trans_filter->next->vector_end(&(trans_filter->vector_meta), trans_filter->next->handler_data);
}

int wk_trans_filter_feature_start(const wk_vector_meta_t* meta, R_xlen_t feat_id, void* handler_data) {
  trans_filter_t* trans_filter = (trans_filter_t*) handler_data;
  trans_filter->recursive_level = -1;
  trans_filter->feature_id++;
  return trans_filter->next->feature_start(&(trans_filter->vector_meta), feat_id, trans_filter->next->handler_data);
}
...
int wk_trans_filter_feature_end(const wk_vector_meta_t* meta, R_xlen_t feat_id, void* handler_data) {
  trans_filter_t* trans_filter = (trans_filter_t*) handler_data;
  return trans_filter->next->feature_end(&(trans_filter->vector_meta), feat_id, trans_filter->next->handler_data);
}

int wk_trans_filter_geometry_start(const wk_meta_t* meta, uint32_t part_id, void* handler_data) {
  trans_filter_t* trans_filter = (trans_filter_t*) handler_data;
...
int wk_trans_filter_geometry_end(const wk_meta_t* meta, uint32_t part_id, void* handler_data) {
  trans_filter_t* trans_filter = (trans_filter_t*) handler_data;
  wk_meta_t* new_meta = trans_filter->meta + trans_filter->recursive_level;
  trans_filter->recursive_level--;
  return trans_filter->next->geometry_end(new_meta, part_id, trans_filter->next->handler_data);
}

int wk_trans_filter_ring_start(const wk_meta_t* meta, uint32_t size, uint32_t ring_id, void* handler_data) {
  trans_filter_t* trans_filter = (trans_filter_t*) handler_data;
  wk_meta_t* new_meta = trans_filter->meta + trans_filter->recursive_level;
  return trans_filter->next->ring_start(new_meta, size, ring_id, trans_filter->next->handler_data);
}

int wk_trans_filter_ring_end(const wk_meta_t* meta, uint32_t size, uint32_t ring_id, void* handler_data) {
  trans_filter_t* trans_filter = (trans_filter_t*) handler_data;
  wk_meta_t* new_meta = trans_filter->meta + trans_filter->recursive_level;
  return trans_filter->next->ring_end(new_meta, size, ring_id, trans_filter->next->handler_data);
}

int wk_trans_filter_coord(const wk_meta_t* meta, const double* coord, uint32_t coord_id, void* handler_data) {
  trans_filter_t* trans_filter = (trans_filter_t*) handler_data;
  wk_meta_t* new_meta = trans_filter->meta + trans_filter->recursive_level;
  ...
  int result = trans_filter->trans->trans(
      trans_filter->feature_id,
      trans_filter->xyzm_in,
      trans_filter->xyzm_out,
      trans_filter->trans->trans_data
  );
  ...
  return trans_filter->next->coord(new_meta, trans_filter->coord_out, coord_id, trans_filter->next->handler_data);
}
...
SEXP wk_c_trans_filter_new(SEXP handler_xptr, SEXP trans_xptr) {
  ...
  if (trans_filter->next->api_version != 1) {
    Rf_error("Can't run a wk_handler with api_version '%d'", trans_filter->next->api_version); // # nocov
  }
